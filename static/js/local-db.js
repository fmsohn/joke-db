/**
 * Local IndexedDB storage for Joke DB (mobile / this-device-only mode).
 * Same conceptual schema as server: jokes, ideas, sets, set_jokes.
 */
(function () {
  "use strict";
  ...
  window.JokeDBLocal = {
        ...
        return withStore(db, "jokes", "readwrite", function (store, tx) {
          return deleteJoke(store, tx, id).then(function (ok) {
            if (ok) {
              ...
              req.onsuccess = function () {
                var c = req.result;
                if (c && c.value.joke_id === parseInt(id, 10)) {
                  c.delete();
                }
                if (c) c.continue();
              };
    ...
    getIdea: function (id) {
      return openDB().then(function (db) {
        return new Promise(function (resolve, reject) {
          var req = db.transaction("ideas", "readonly").objectStore("ideas").get(parseInt(id, 10));
          req.onsuccess = function () {
            var r = req.result;
            resolve(r && r.comedian_id === COMEDIAN_ID ? r : null);
          };
          req.onerror = function () { reject(req.error); };
        });
      });
    ...
    reorderSetJokes: function (setId, jokeIds) {
      return openDB().then(function (db) {
        return reorderSetJokes(db, setId, jokeIds).then(function () {
          return window.JokeDBLocal.getSetWithJokes(setId);
        });
      });
    },
